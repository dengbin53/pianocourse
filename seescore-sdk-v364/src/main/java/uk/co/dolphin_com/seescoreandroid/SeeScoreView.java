...
public class SeeScoreView extends LinearLayout {
     ...
     * @param cursorView The view presenting the cursor.
     ...
     */
    public SeeScoreView(Activity context, CursorView cursorView, AssetManager am, ZoomNotification zn, TapNotification tn) {
        super(context);
    ...
    private void addSystem(final SSystem sys) {
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                SystemView sv = new SystemView(getContext(), cursorView, score, sys, SeeScoreView.this.assetManager, tapNotify);
                addView(sv);
                views.add(sv);
            }
        });
    ...
    public void clear(final Runnable completion) {
        ...
        abortLayout(new Runnable() {
            public void run() {
                ...
                if (score != null && getWidth() > 0) {
                    layout(score);
                    setCursorAtBar(0, SeeScoreView.CursorType.line, 0);
                } else// else layout will be called in OnSizeChanged()
                {
                    systems = null;
                    removeAllViews();
                }
    ...
    private void slowSmoothScrollTo(final ScrollView parentScrollView, int scrollY, int autoScrollAnimationTime) {
        ObjectAnimator.ofInt(parentScrollView, "scrollY", scrollY).setDuration(autoScrollAnimationTime).start();
    ...
    public boolean setCursorAtBar(int barIndex, CursorType type, int autoScrollAnimationTime) {
        ...
        for (View view : views) {
            if (view instanceof SystemView
                    && view.isShown()) {
                SystemView systemView = (SystemView) view;
                if (systemView.setCursorAtBar(barIndex, (type == CursorType.line) ? CursorView.CursorType.line : CursorView.CursorType.box)) {
        ...
        if (system != null) {
            try {
                ...
                for (Component comp : components) {
                    if (comp.type == Component.Type.notehead || comp.type == Component.Type.rest)
                        return comp.rect.left + comp.rect.width() / 2; // centre of notehead/rest
                }
    ...
    public void moveNoteCursor(List<Note> notes, int animationTime) {
        // 1. find first non-rest
        for (Note note : notes) {
            int barIndex = note.startBarIndex;
            if (note.start >= 0 // ignore cross-bar tied notes
                    && !note.rest // ignore rests if possible
                    && note.grace == Note.Grace_No) // ignore grace notes which are too fast for animation
            {
        ...
        // 2. find first rest if no notes available
        for (Note note : notes) {
            int barIndex = note.startBarIndex;
            if (note.start >= 0 // ignore cross-bar tied notes
                    && note.grace == Note.Grace_No) // ignore grace notes which are too fast for animation
    ...
    private class LayoutThread extends Thread {
        LayoutThread(SScore score, float displayHeight) {
            super("LayoutThread");
            this.score = score;
    ...
    private Stack<Runnable> abortStack = new Stack<>();
    ...
    private void layout(final SScore score) {
        if (layoutThread == null) {
            systems = new SSystemList();
            removeAllViews();
            layoutThread = new LayoutThread(score, screenHeight);
            layoutThread.start();
        }
    }
    ...
    public void colourItem(int partIndex, int barIndex, int item_h, RenderItem.Colour col, boolean single_system_colouring) {
        for (View view : views) {
            if (view instanceof SystemView
                    && view.isShown()) {
                SystemView systemView = (SystemView) view;
                if (systemView.containsBar(barIndex)) {
                    ...
                    if (!single_system_colouring)
                        return;
                } else if (single_system_colouring) {
