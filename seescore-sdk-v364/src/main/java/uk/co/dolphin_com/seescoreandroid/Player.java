...
public class Player {
     ...
     * @param playControls      an interface to the UI for determining parameters for playing
     * @param startLoopBarIndex the bar index for the first bar of the loop
     * @param endLoopBarIndex   the bar index for the last bar of the loop
     * @param numRepeats        the number of repeats of the loop, or zero if not looping NB The MIDI file grows in proportion to this parameter, so don't set it too high!
    ...
    /**
     * Construct the Player without looping
     *
     * @param score        the score to play
     * @param userTempo    access to UI (eg slider) which defines the tempo
     * @param context      the context (eg MainActivity)
    ...
    /**
     * recreate the MIDI file to be called on changing any parameter
     */
    ...
    /**
     * min bar time of less than 1s requires fast cursor
     ...
     *
     * @return true if the min bar time is less than 1s
     */
    ...
    public int bestScrollAnimationTime() {
        ...
        if (minBarTime < 2000)
            return minBarTime / 4; // for min bar less than 2s use 1/4 bar time so animation isn't slower than bar time
        else
            return 500; // 0.5s is the maximum time to animate the cursor
    }
     ...
     * restart playing
     *
     * @param countIn if set insert a count-in bar
     */
