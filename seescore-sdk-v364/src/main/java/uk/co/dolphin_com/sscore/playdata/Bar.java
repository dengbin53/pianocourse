...
public class Bar {
    ...
    {
        return new Part(nativePointer, sequenceIndex, countIn, partIndex, loop);
    }
    ...
    {
        return new Part(nativePointer, sequenceIndex, countIn, kMetronomePartIndex, loop);
    }
    ...
    Bar(long nativePointer, int idx,  int dur, int seqi, boolean ci, long loop) {
        this.nativePointer = nativePointer;
        index = idx;
        duration = dur;
        sequenceIndex = seqi;
