...
public class Sound
{
	...
	public String toString()
	{
		...
		if (pizz)
			sb.append(" pizz");
		if (!damper_pedal.isEmpty())
			sb.append(" damper_pedal:").append(damper_pedal);
		if (!soft_pedal.isEmpty())
			sb.append(" soft_pedal:").append(soft_pedal);
		if (!sostenuto_pedal.isEmpty())
			sb.append(" sostenuto_pedal:").append(sostenuto_pedal);
		return sb.toString();
	}

	private Sound(int offset, int tempo, float dynamics, boolean dacapo,
				  String segno, String dalsegno, String coda, String tocoda,
				  boolean forward_repeat, float divisions, String fine,
				  String timeonly, boolean pizz,
	...
	private Sound(int tempo, float dynamics) {
		this.offset = 0;
		this.tempo = tempo;
		this.dynamics = dynamics;
		this.dacapo = false;
